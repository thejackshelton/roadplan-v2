...
globalThis.qwikOptimizer = function(module) {
  ...
  async function loadPlatformBinding(sys) {
    ...
    if ("webworker" === sysEnv || "browsermain" === sysEnv) {
      ...
      if (!cjsCode || !wasmRsp) {
        ...
        const cdnUrl = `https://cdn.jsdelivr.net/npm/@qwik.dev/core@${version2}/bindings/`;
  ...
  function generateManifestFromBundles(path, segments, injections, outputBundles, opts, debug) {
    ...
    const qrlNames = new Set([ ...segments.map((h2 => h2.name)) ]);
  ...
  function createPlugin(optimizerOptions = {}) {
    ...
    const resolveId = async (ctx, id2, importerId, resolveOpts) => {
      ...
      if (parentIds.get(pathId)) {
        ...
        if (qrlMatch) {
          ...
          if (resolvedParent) {
            ...
            console.error(`resolveId(${count2})`, `QRL parent ${parent} does not exist!`);
          }
        } else if (importerId) {
          const importerParentId = parentIds.get(importerId);
          if (importerParentId) {
            debug(`resolveId(${count2}) end`, `resolving via ${importerParentId}`);
      ...
      debug(`resolveId(${count2}) end`, (null == result ? void 0 : result.id) || result);
      return result;
    };
    let loadCount = 0;
      ...
      if (devServer && !outputs.has(id2)) {
        ...
        if (parentId) {
          ...
          if (parentModule) {
            ...
            console.error(`load(${count2})`, `${parentModule} does not exist!`);
          }
        }
      }
    ...
    const createOutputAnalyzer = rollupBundle => {
      ...
      const generateManifest = async () => {
        const optimizer2 = getOptimizer();
        const path = optimizer2.sys.path;
        const segments = Array.from(clientResults.values()).flatMap((r => r.modules)).map((mod => mod.segment)).filter((h2 => !!h2));
  ...
  var vnode_diff = (container, jsxNode, vStartNode, scopedStyleIdPrefix) => {
    ...
    function expectComponent(component) {
      ...
      if (componentMeta) {
        ...
        null != jsxNode2.children && descendContentToProject(jsxNode2.children, host);
      } else {
    ...
    function insertNewInlineComponent() {
      ...
      jsxNode2.key && vnode_setProp(vNewNode, ELEMENT_KEY, jsxNode2.key);
    }
  ...
  function getComponentHash(vNode, getObject) {
    ...
    const qrl2 = vnode_getProp(vNode, OnRenderProp, getObject);
      ...
      switch (chore.$type$) {
       ...
       case 5:
        ...
        if (isConst) {
          ...
          journal.push(2, element, property, serializedValue);
        } else {
  ...
  var inflate = (container, target, typeId, data) => {
    ...
    switch (typeId) {
     ...
     case 29:
      ...
      propsProxy[_VAR_PROPS] = 0 === data ? {} : data[0];
  ...
  var allocate = (container, typeId, value) => {
    ...
    switch (typeId) {
     ...
     case 18:
      const qrl2 = container.$getObjectById$(value);
      return parseQRL(qrl2);
  ...
  function parseQRL(qrl2) {
    ...
    const chunk = hashIdx > -1 ? qrl2.slice(0, hashIdx) : qrl2.slice(0, captureStart);
  ...
  function inflateQRL(container, qrl2) {
    ...
    qrl2.$captureRef$ = captureIds ? captureIds.map((id => container.$getObjectById$(id))) : null;
    ...
    async function breakCircularDependenciesAndResolvePromises() {
      ...
      const visit = obj => {
        if ("function" === typeof obj) {
          ...
          v !== NEEDS_COMPUTATION && discoveredValues.push(v);
    ...
    const writeValue = (value, idx) => {
      if (fastSkipSerialize(value)) {
        ...
        if (value === Slot) {
          ...
          serializationContext.$renderSymbols$.add(qrl2.$symbol$);
          output(21, [ qrl2 ]);
        } else {
  ...
  var createQRL = (chunk, symbol, symbolRef, symbolFn, capture, captureRef, refSymbol) => {
    ...
    symbolRef && (symbolRef = maybeThen(symbolRef, (resolved => qrl2.resolved = symbolRef = wrapFn(resolved))));
    isDev && Object.defineProperty(qrl2, "_devOnlySymbolRef", {
      get: () => symbolRef
    });
    qDev && seal(qrl2);
    return qrl2;
    ...
    server.middlewares.use((async (req, res, next) => {
      try {
        ...
        if (shouldSsrRender(req, url)) {
          ...
          const render2 = ssrModule.default ?? ssrModule.render;
          if ("function" === typeof render2) {
            const manifest = {
              manifestHash: "",
              symbols: {},
              mapping: {},
            ...
            res.setHeader("X-Powered-By", "Qwik Vite Dev Server");
            res.writeHead(status);
            const result = await render2(renderOpts);
            Array.from(server.moduleGraph.fileToModulesMap.entries()).forEach((entry => {
              entry[1].forEach((v => {
                const {pathId: pathId, query: query} = parseId(v.url);
